...
const STORAGE_KEY = "jsoncrack-content"

const defaultJson = JSON.stringify(
  {
    fruit: { name: "Apple", color: "Red", weight: "150g" },
    car:   { model: "Model S", year: 2022, brand: "Tesla" },
    person:{ name: "Alice", occupation: "Engineer", age: 30 },
  },
  null,
  2
)
...
const initialStates = {
  ...
  contents: savedContent ?? defaultJson,
  error: null as any,
  hasChanges: false,
...
const isURL = (value: string) =>
  /(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][\w-]+\.[^\s]{2,})/.test(value)
...
const useFile = create<FileStates & JsonActions>()((set, get) => ({
  ...
  setContents: async ({
    ...
    try {
      ...
      localStorage.setItem(STORAGE_KEY, get().contents)
